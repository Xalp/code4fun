...
This is a dedicated evaluation script for Dream with SMC support.
Unlike the combined eval.py, this properly uses diffusion_generate with
the SMC block generation mixin.

Usage:
    accelerate launch --num_processes 8 dllm/examples/dream_smc/eval.py \
        --tasks gsm8k \
        --model dream_smc \
        --apply_chat_template \
        --num_fewshot 5 \
        --model_args "pretrained=Dream-org/Dream-v0-Instruct-7B,max_new_tokens=256,steps=256,use_smc=true,temperature=1.0"
"""
...
# Import Dream model and generation mixins
...
class DreamSMC(LM):
    def __init__(
        ...
        temperature: Optional[float] = 1.0,  # Default 1.0 for SMC
        ...
        use_cache: Optional[bool] = True,  # Enable cache by default
        use_smc: Optional[bool] = True,    # Enable SMC by default
        ...
        **kwargs,
        ...
        if isinstance(pretrained, str):
            if gpus >= 1 or str(self.device) == "mps":
                if not (parallelize or autogptq or hasattr(self, "accelerator") and self.accelerator is not None):
                    try:
                ...
                if gpus > 1:
                    if accelerator.num_processes > 1:
                        if parallelize:
                            eval_logger.warning(
                                "You are both using a HF Accelerate `device_map` and launching via `accelerate launch`. This will attempt to do model and data parallelism."
                            )
                        elif gpus > accelerator.num_processes:
                            eval_logger.warning(
                                "The number of total system GPUs does not match the number of spawned processes. "
        ...
        # Convert string bools from command line
        if isinstance(use_smc, str):
            use_smc = use_smc.lower() == "true"
        if isinstance(use_cache, str):
            use_cache = use_cache.lower() == "true"
        if isinstance(dual_cache, str):
            dual_cache = dual_cache.lower() == "true"
        ...
        # Dynamically bind generation methods based on use_smc/use_cache
    ...
    def _generate_batch(self, prompts: List[str]) -> List[str]:
        ...
        if len(prompt_ids[0]) > self.max_length - self.max_new_tokens:
            eval_logger.warning(f"Prompt length {len(prompt_ids[0])} is larger than {self.max_length-self.max_new_tokens}, cutoff on the left side")
